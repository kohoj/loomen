...
- Continue splitting `src-tauri/src/main.rs` into domain modules; Pulse command discovery/evidence, PTY terminal sessions/snapshots, diff review parsing/comments, GitHub PR/check operations, sidecar process/RPC streaming, Git execution/checkpoints/worktree operations, settings defaults/persistence, database schema/migrations, and snapshot read-model queries now live behind Rust module boundaries, with remaining persistence commands and app bootstrap still to follow.
