...
- Continue splitting `src-tauri/src/main.rs` into domain modules; Pulse command discovery/evidence, PTY terminal sessions/snapshots, and diff review parsing/comments now live behind Rust module boundaries, with persistence, git, sidecar, GitHub, settings, and app bootstrap still to follow.
