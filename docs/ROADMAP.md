...
- Continue splitting `src-tauri/src/main.rs` into domain modules; Pulse command discovery/evidence, PTY terminal sessions/snapshots, diff review parsing/comments, GitHub PR/check operations, and sidecar process/RPC streaming now live behind Rust module boundaries, with persistence, git, settings, and app bootstrap still to follow.
